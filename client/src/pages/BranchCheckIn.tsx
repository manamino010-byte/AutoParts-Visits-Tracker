...
export default function BranchCheckIn() {
  ...
  const openExternalMissionModal = () => {
    ...
    // ✅ Bug Fix: المأمورية الخارجية بتتسجل مباشرة على السيرفر (مفيش offline queue ليها)
    // بنمنع المحاولة أوفلاين عشان ميجيش error مش مفهوم للمدير
    if (!navigator.onLine) {
      return toast.error("📶 المأمورية الخارجية تحتاج اتصالاً بالإنترنت — تأكد من النت وحاول تاني");
    }
  ...
  const submitModal = async () => {
    ...
    // ✅ التحقق من سبب المأمورية بيتم الآن في NotesModal (زر disabled لو مفيش سبب مختار)
    // visitNotes بتيجي مدموجة مع السبب: "[سبب المأمورية] - تفاصيل إضافية"
