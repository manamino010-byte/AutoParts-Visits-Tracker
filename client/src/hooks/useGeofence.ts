...
export function useGeofence() {
  ...
  useEffect(() => {
    if (Capacitor.isNativePlatform()) {
      // ✅ Bug Fix: branchId = null للمأمورية الخارجية — null.toString() كان بيكسر الأب
      // لو في زيارة نشطة ولها فرع حقيقي → احفظ الـ ID، وإلا (مأمورية خارجية أو مفيش زيارة) → امسح
      if (activeVisit?.branchId != null) {
