...
const visitHistorySelection = {
  ...
  // ✅ حالة اعتماد المأمورية — المدير يشوفها في سجل الزيارات وعلى الداشبورد
...
export const visitRouter = router({
  ...
  checkIn: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // ✅ مسافة المأمورية من الزيارة السابقة — تتحسب وقت checkIn (لو مفيش زيارة سابقة = null)
      let missionDistanceToPrevKm: string | undefined;
      if (input.visitType === "external_mission") {
        // ✅ نعيد استخدام checkTravelFromPrevBranch لحساب المسافة من الزيارة السابقة
        // (نفس الـ call اللي فوق لكن نحتاج النتيجة هنا — الـ teleport check السابق شغّال)
        // لو مفيش زيارة سابقة في نفس اليوم → km = null → مانحفظش المسافة
        const travelForDist = await checkTravelFromPrevBranch(
          db, manager.id, "مأمورية خارجية",
          parseFloat(input.latitude), parseFloat(input.longitude),
          new Date()
        );
        if (travelForDist.km !== null) {
          missionDistanceToPrevKm = travelForDist.km.toFixed(2);
        }
      }
      ...
      await db.insert(visits).values({
        ...
        // ✅ مسافة المأمورية من الزيارة السابقة (محسوبة وقت checkIn)
        ...(missionDistanceToPrevKm !== undefined ? { distanceToPrevBranchKm: missionDistanceToPrevKm } : {}),
        ...
        ...(input.visitType === "external_mission" ? {
          ...
          // ✅ تم إزالة أقرب فرع بناءً على طلب المستخدم
      ...
      // 🧭 لو مأمورية خارجية — إشعار كامل للأدمن بانتظار المراجعة + إيميل (fire-and-forget)
      if (input.visitType === "external_mission") {
        const managerName = await getManagerName(db, ctx.user!.id);
        notifyExternalMissionPending(
          db, managerName, input.notes,
  ...
  myHistory: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      // ✅ المدير يشوف كل المأموريات بما فيها المرفوضة — عشان يعرف حالتها
  ...
  getActive: protectedProcedure.query(async ({ ctx }) => {
    ...
    const items = await db.select(visitHistorySelection)
      ...
      .where(and(
        ...
        eq(visits.status, "checked_in"),
        // ✅ المأمورية المرفوضة لا تظهر كزيارة نشطة للمدير (فقط من getActive)
